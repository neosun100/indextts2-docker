...
IndexTTS2は話者埋め込みベクトルのためのGPUメモリキャッシュを内蔵：

**パフォーマンス向上**：
- **同じ話者を再利用する場合、3.4%高速化**
- 初回呼び出し：5.436秒（埋め込み抽出）
- 後続呼び出し：5.207秒（キャッシュ使用、0.229秒節約）
- テスト条件：5人の話者 × 5回の反復（外れ値除去済み）

**GPU メモリ (VRAM)**：
- 固定割り当て：約13GB、話者数に関係なく一定
- 単一スロットキャッシュ：一度に1人の話者のみ保存
- 自動クリーンアップ：古い埋め込みは自動的に上書き
- **メモリ蓄積なし** - 無制限の話者数をサポート

**メモリ安全性保証**：
```
✅ GPUメモリ：固定約13GB（増加しない）
✅ VRAMの肥大化なしで無制限の話者をサポート
✅ 自動キャッシュ管理を内蔵
✅ 繰り返し話者で3.4%のパフォーマンス向上
```
